...
function CredentialsForm({
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      ...
      <div>
        <label className="auth-label" htmlFor="password">{t("auth.signIn.passwordLabel")}</label>
        <input
          id="password"
          type="password"
...
function AuthFooter({ callbackUrl }: { callbackUrl: string }) {
  const t = useTranslations();
  return (
    <>
  ...
  return (
    <div className="auth-page">
      <div className="auth-card">
        ...
        <AuthFooter callbackUrl={callbackUrl} />
      </div>
    </div>
  );
}
